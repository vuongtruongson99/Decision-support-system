...
print("\n---------------------------------------")
print("\tDuality problem")

A_np = np.array(A_new)
Cb = np.array(Cb).reshape(1, 2)
diagonal_matrix = np.eye(2)

A_np = np.concatenate((A_np, diagonal_matrix), axis=1)

index = [int(x[-1]) - 1 for x in Basic]
D = A_np[:, index]
D_inv = np.linalg.inv(D)

y = Cb.dot(D_inv).tolist()

for ind, val in enumerate(y[0]):
    print("y" + str(ind+1) + " = " + str(val))

ans = 0
for i in range(len(y[0])):
    ans += (B_duality[i] * y[0][i])
